    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.getSerializableExtra("data") != null) {
            ...
            if (!orderData.restaurant.latitude.isEmpty() && !orderData.restaurant.longitude.isEmpty() && orderData.userAddress.latitude!=0.0 && orderData.userAddress.longitude!=0.0) {
                val restaurantToUser = distance(orderData.restaurant.latitude.toDouble(), orderData.restaurant.longitude.toDouble(), orderData.userAddress.latitude, orderData.userAddress.longitude)
            ...
            val streetName= if (!orderData.userAddress.streetName.isEmpty()) "/"+orderData.userAddress.streetName else ""
            val landmark=if (!orderData.userAddress.deliveryInstructions.isEmpty()) "\n" +orderData.userAddress.deliveryInstructions else ""
            val userAddres=if (!orderData.userAddress.address.isEmpty()) "\n" +orderData.userAddress.address else ""
